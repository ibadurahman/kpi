...
class Breadcrumbs {
	...
	{	
		...
		$this->inner_link_open = $this->ci->config->item('inner_link_open');
		$this->inner_link_close = $this->ci->config->item('inner_link_close');
		$this->class_link = $this->ci->config->item('class_link');
	...
	function show()
	{
        ...
        /**
	 * Generate breadcrumb custom untuk menyesuaikan design web
	 *
	 * @access	public
	 * @return	string
	 */		
	function show_custom()
	{
		if ($this->breadcrumbs) {
			...
			foreach ($this->breadcrumbs as $key => $crumb) {
				$keys = array_keys($this->breadcrumbs);
				if (end($keys) == $key) {
                                        $output .= $this->crumb_last_open.'<a href="' . $crumb['href'] . '" class="'.$this->class_link.'">'.$this->inner_link_open . $crumb['page'] .$this->inner_link_close. '</a> '.$this->crumb_close;
				} else {
					$output .= $this->crumb_open.'<a href="' . $crumb['href'] . '" class="'.$this->class_link.'">'.$this->inner_link_open . $crumb['page'] .$this->inner_link_close. '</a> '.$this->crumb_close.$this->crumb_divider;
				}
