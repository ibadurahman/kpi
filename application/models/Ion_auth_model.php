...
class Ion_auth_model extends CI_Model
{
	...
	public function __construct()
	{
		...
		$this->identity_2_column = $this->config->item('identity_secondary', 'ion_auth');
	...
	public function change_password($identity, $old, $new)
	{
		...
		$query = $this->db->select('id, password')
		                  ->where($this->identity_column, $identity)
                                  ->or_where($this->identity_2_column, $identity)
		                  ->limit(1)
		                  ->order_by('id', 'desc')
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ', email, id, password, active, last_login, nip,first_name,last_name, image')
						  ->where($this->identity_column, $identity)
                                                  ->or_where($this->identity_2_column, $identity)
						  ->limit(1)
						  ->order_by('id', 'desc')
        ...
        public function login_app($identity, $password, $remember=FALSE)
	{
		$this->trigger_events('pre_login');
		if (empty($identity) || empty($password))
		{
	...
	public function set_session($user)
	{
		$this->trigger_events('pre_set_session');
                ...
                $foto2="";
                if($user->nip != ""){
                    $query = $this->db->where('nip', $user->nip)
                                                      ->limit(1)
                                                      ->order_by('nip', 'desc')
                                                      ->get('pegawai');

                    if($query->num_rows()>0){
                        $DataPegawai=$query->row();
		...
		$this->trigger_events('pre_set_session');
                ...
                if($user->nip != ""){
                    ...
                    if($query->num_rows()>0){
                        $DataPegawai=$query->row();
                        $nama=$DataPegawai->nama;
                        $foto=base_url('assets/upload/foto/'.$DataPegawai->foto);
                    }
                }
                if($foto==""){
                    $foto=$foto2;
                }
                $DataGroup= $this->get_users_groups($user->id);
                $IdGroup="";
                $NmGroup="";
...
//		    'last_check'           => time(),
		    ...
		    'login_nama'           => $nama,
		    'login_foto'           => $foto,
		    'kd_user_role'           => $IdGroup,
		    'nm_user_role'           => $NmGroup,
		    //'login_foto_user'           => $foto2,
		];
	...
	public function remember_user($identity)
	{
		...
		if ($token->validator_hashed)
		{
                        ...
                        $this->db->or_where($this->identity_2_column,$identity);
...
//                                delete_cookie($this->config->item('remember_cookie_name', 'ion_auth'));
				...
				//set_cookie($this->config->item('remember_cookie_name', 'ion_auth'),$token->user_code,time()+$expire);
	...
	public function login_remembered_user()
	{
		...
		$this->trigger_events('extra_where');
		$query = $this->db->select($this->identity_column . ', id, email, remember_code, last_login, nip, first_name, last_name, image')
						  ->where('remember_selector', $token->selector)
						  ->where('active', 1)
						  ->limit(1)
						  ->get($this->tables['users']);
//                var_dump($remember_cookie);
