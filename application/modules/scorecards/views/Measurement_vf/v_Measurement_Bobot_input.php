<?php 
if ( ! defined('BASEPATH')) exit('No direct script access allowed');
$RowData=$DataMeasurement->row();
?>
<div class="m-form__heading">
	<h3 class="m-form__heading-title"><?php echo $this->lang->line('header_bobot_title')." ".$RowData->nm_bd." (".$RowData->kd_bds.")"; ?></h3>
</div>
<div class="form-group m-form__group">
	<?php echo form_label($this->lang->line('tahun')."<span class='m--font-danger'>*</span>",'tahun');?>
        <p class="form-control-static"><?php echo $Tahun; ?></p>
        <?php echo form_hidden("tahun",$Tahun);?>
        <div class="error"></div>
</div>
<div class="form-group m-form__group">
	<?php echo form_label($this->lang->line('weightage')."<span class='m--font-danger'>*</span>",'weightage');?>
    <div class="m-section__content " id="alert_error_section"></div>
        <table class="table table-bordered table-hover">
            <thead>
                    <tr>
                            <th>#</th>
                            <th width="100"><?php echo $this->lang->line('kd_measurement');?></th>
                            <th><?php echo $this->lang->line('nm_measurement');?></th>
                            <th width="150"><?php echo $this->lang->line('weightage');?></th>
                            <th width="100"><?php echo "Total (%)";?></th>
                    </tr>
            </thead>
            <tbody>
                <?php
                $no=1;
                $Total=0;
                $Total_persen=0;
                foreach($DataMeasurement->result() as $row)
                {
                    $Total=$Total+$row->weightage;
                    $Total_persen=$Total_persen+$row->bobot;
                ?>
                    <tr>
                            <td scope="row"><?php echo $no; ?></td>
                            <td>
                                <?php echo form_input(array(
                                                            'name'          => 'kd_ms[]',
                                                            'id'            => 'kd_ms'.$no,
                                                            'class'         => 'form-control m-input ',
                                                            'placeholder'    => $this->lang->line('kd_ms'),
                                                            'readonly'      => true
                                                    ),set_value('kd_ms'.$no,$row->kd_ms));
                                
                                        echo form_input(array(
                                                            'name'          => 'kd_measurement[]',
                                                            'id'            => 'kd_measurement'.$no,
                                                            'class'         => 'form-control m-input ',
                                                            'placeholder'    => $this->lang->line('kd_measurement'),
                                                            'type'      => 'hidden'
                                                    ),set_value('kd_measurement'.$no,$row->kd_measurement));            
                                ?>
                                <div class="error"></div>
                            </td>
                            <td>
                                <?php echo form_input(array(
                                                            'name'          => 'nm_measurement[]',
                                                            'id'            => 'nm_measurement'.$no,
                                                            'class'         => 'form-control m-input',
                                                            'placeholder'    => $this->lang->line('nm_measurement'),
                                                            'readonly'      => true
                                                    ),set_value('nm_measurement'.$no,$row->nm_measurement));
                                ?>
                                <div class="error"></div>
                            </td>
                            <td>
                                <?php echo form_input(array(
                                                            'name'          => 'weightage[]',
                                                            'id'            => 'weightage'.$no,
                                                            'class'         => 'form-control m-input list-weightage',
                                                            'placeholder'    => $this->lang->line('weightage'),
                                                            'type'  => 'number'
                                                    ),set_value('weightage'.$no,$row->weightage));
                                ?>
                                <div class="error"></div>
                            </td>
                            <td scope="row" align="right" id="<?php echo "bobot".$no; ?>"><?php echo $row->bobot."%"; ?></td>
                    </tr>
                <?php 
                        $no++;
                    }
                ?>
                    <tr>
                        <td colspan="3" align="right"><b>Total</b></td>
                        <td id="total"><b><?php echo $Total; ?></b></td>
                        <td id="total_persen" align="right"><b><?php echo $Total_persen."%"; ?></b></td>
                        <?php echo form_input(array(
                                                            'name'          => 'total_bobot',
                                                            'id'            => 'total_bobot',
                                                            'type'  => 'hidden'
                                                    ),set_value('total_bobot',$Total_persen));
                                ?>
                    </tr>
            </tbody>
        </table>
</div>

