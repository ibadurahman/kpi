                ...
                // Add series
                 series : [
                        { 
                            ...
                            data:[{value: <?php echo $gauge_chart; ?>, name: 'Score'}]
                        }]
            };
...
//
        ...
        function (ec) {
            ...
            chartOptions = {
                ...
                legend: {
                    ...
                    right: 10,
                    <?php
                        if(isset($line_chart['legend'])){
                        $legend="'".implode("','", $line_chart['legend'])."'";
                        echo "data: [".$legend."],";
                        }
                    ?>
                },
                ...
                // Add custom colors
                <?php
                    if(isset($line_chart['color'])){
                        $color="'".implode("','", $line_chart['color'])."'";
                        echo "color: [".$color."],";
                    }
                    ?>
                // Enable drag recalculate
                ...
                xAxis: [{
                    ...
                    boundaryGap: false,
                    <?php
                        if(isset($line_chart['bulan'])){
                        $bulan="'".implode("','", $line_chart['bulan'])."'";
                        echo "data: [".$bulan."]";
                        }
                    ?>
                }],
                ...
                series: [
                    <?php
                    if(isset($line_chart['data_grafik'])){
                    foreach($line_chart['data_grafik'] as $key=>$val){
 ...
 $(document).ready(function(){
    ...
    if(isset($data_kpi))
    {
        ...
        foreach($data_kpi->result() as $row)
        {
            $Color=$ListColor[array_rand($ListColor,1)];
    ?>
    var e = new Chartist.Pie("#m_chart_<?php echo $row->kd_measurement; ?>", {
                    series: [{
                        value: <?php echo $row->gross_result; ?>,
                        className: "<?php echo addslashes($row->nm_measurement); ?>",
                        meta: {
